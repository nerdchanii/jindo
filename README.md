...
Jindo의 TUI는 React 기반 ink 라이브러리로 구현된 현대적인 터미널 인터페이스입니다.

### TUI 특징

- 🎨 **현대적 UI**: React 기반으로 빠르고 반응성이 뛰어남
- ⌨️ **키보드 내비게이션**: 직관적인 단축키 지원
- 💡 **자동완성**: Tab 키로 명령어 및 텍스트 자동완성
- 📊 **실시간 정보**: 상태 표시 및 진행 상황 바로 확인
- 🎯 **슬래시 명령어**: `/help`, `/status`, `/model` 등 빠른 명령어

### TUI 키보드 단축키

| 키       | 기능            | 설명                  |
| -------- | --------------- | --------------------- |
| `Tab`    | 자동완성 토글   | 제안 목록 표시/숨김   |
| `↑↓`     | 제안 내비게이션 | 자동완성 항목 선택    |
| `Enter`  | 메시지 전송     | 현재 입력 메시지 전송 |
| `Esc`    | 제안 취소       | 자동완성 목록 숨김    |
| `Ctrl+C` | 입력 지우기     | 현재 입력 내용 초기화 |
| `←→`     | 제안 취소       | 자동완성 목록 숨김    |
...
💬 Messages: 1
🔌 MCP Servers: 0 enabled
📚 Providers: 0

──────────────────────────────────────────────────────────────────
Tab: Suggestions • Ctrl+C: Clear • Esc: Cancel
Message... (Tab for autocomplete)_
```

### TUI 사용 권장

TUI 모드는 CLI 모드에 비해 다음과 같은 장점이 있습니다:

1. **더 나은 사용자 경험**: 반응성 있는 UI와 직관적인 상호작용
2. **자동완성**: 명령어와 일반 텍스트 자동완성 지원
3. **실시간 피드백**: 상태 정보와 진행 상황 실시간 표시
4. **키보드 중심**: 마우스 없이 모든 작업 가능
