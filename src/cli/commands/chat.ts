...
export const chatCommand = new Command('chat')
  ...
  .action(async (options: { format?: string; verbose?: boolean; streaming?: boolean }) => {
    ...
    const modelSelector = new ModelSelector({
      conversationModel: config.agent.conversationModel.replace('ollama:', ''),
      functionModel: config.agent.functionModel.replace('ollama:', ''),
    });
    ...
    try {
      ...
      if (!ready) {
        ...
        console.log(chalk.gray(`  ollama pull ${config.agent.conversationModel.replace('ollama:', '')}`));
        console.log(chalk.gray(`  ollama pull ${config.agent.functionModel.replace('ollama:', '')}`));
        process.exit(1);
      }

      initSpinner.succeed('Agent ready');
    } catch (error) {
      initSpinner.fail(`Failed to initialize: ${error instanceof Error ? error.message : String(error)}`);
      process.exit(1);
    }
      ...
      try {
        if (streaming) {
          ...
          if (response.toolCalls && response.toolCalls.length > 0) {
            cli.writeLine(chalk.gray(`  [Tools used: ${response.toolCalls.map(t => t.name).join(', ')}]`));
          }

          if (options.verbose) {
            cli.writeDebug(`Tokens: ${response.tokens.totalTokens}, Time: ${response.executionTime}ms`);
          }
        }
      } catch (error) {
